...
Source: [[chan-pre-release-phase-2/webtest-2.md]] flagged three
remaining smoke probes and one open finding pending @@Architect
direction. [[chan-pre-release-phase-2/architect-8.md]] already
...
* [[chan-pre-release-phase-2/webtest-2.md]] REVIEW — accepted for
  ...
  [[chan-pre-release-phase-2/webtest-smoke.mjs]] is the canonical
  phase-2 smoke entry point.
* Service note at [[chan-pre-release-phase-2/webtest-1.md]] is
...
[[chan-pre-release-phase-2/webtest-1.md]] under a new dated
section. The real fix (chan-report reconcile-on-load) is filed
as the non-blocking [[chan-pre-release-phase-2/backend-5.md]];
do not block on it.
...
|---------------------------------|-------------|----------|
| G1a ghost-while-open            | [[chan-pre-release-phase-2/backend-3.md]] | delete an indexed `.md` from `Scratch/phase2-smoke/` while the graph overlay is open; the node renders `missing: true` ghost styling without manual reload |
| G4 live-add-while-open          | [[chan-pre-release-phase-2/frontend-7.md]] | create a `.md` in `Scratch/phase2-smoke/` while the overlay is open; the new node appears within the 250ms debounce |
| G3 depth-slider scope-aware cap | [[chan-pre-release-phase-2/frontend-9.md]] | switch scope file -> group -> dir -> drive; the slider's `max` attribute reflects the documented cap rules (1, N, dir depth, drive depth clamped at 6) |
...
[[chan-pre-release-phase-2/webtest-2.md]] under a new dated
