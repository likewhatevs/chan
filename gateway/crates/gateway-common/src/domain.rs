...
//!
//! identity-service and devserver-proxy must agree on the public
//! hostnames, or the devserver-gate handoff breaks: the entry/session
//! JWT `aud` claim is the inbound host, and devserver-proxy parses the
...
//! so the id and devserver hosts cannot drift apart.
//!
...
/// `chan.app`). The `id.` / `devserver.` prefixes are the fixed chan
...
pub struct Domains {
    ...
    pub id_host: String,
    /// devserver-proxy apex host (e.g. `devserver.chan.app`).
    pub devserver_apex: String,
...
impl Domains {
    ...
    pub fn from_base(base: &str) -> Self {
        ...
        Self {
            ...
            devserver_apex: format!("devserver.{base}"),
            devserver_wildcard_suffix: format!(".devserver.{base}"),
            base,
        }
    }
...
mod tests {
    ...
    fn derives_prod_hosts() {
        ...
        assert_eq!(d.devserver_apex, "devserver.chan.app");
        assert_eq!(d.devserver_wildcard_suffix, ".devserver.chan.app");
    }
    ...
    fn dev_default_base() {
        ...
        assert_eq!(d.devserver_apex, "devserver.localtest.me");
    }
}
