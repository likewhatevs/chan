...
Shared internal library for the chan-gateway crates: typed HTTP clients, the devserver-gate JWT, hostname derivation, validators, and the token-bucket throttle primitive.
...
- `domain`: public hostnames (`id.<base>`, `devserver.<base>`, `.devserver.<base>`) derived from `CHAN_DOMAIN`; identity and workspace-proxy use it so the devserver-gate `aud` cannot drift.
