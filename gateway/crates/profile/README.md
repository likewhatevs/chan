...
| `WORKSPACE_ADMIN_URL`  | no       | devserver-proxy admin base; set with the token |
| `WORKSPACE_ADMIN_TOKEN`| no       | enables the admin-block fan-out that evicts the user's tunnels |

A missing `PROFILE_ADMIN_TOKEN` makes every `/v1/admin/*` route return 401; that is the safe default for a fresh deploy. When `WORKSPACE_ADMIN_URL` + `WORKSPACE_ADMIN_TOKEN` are set, blocking a user also tells devserver-proxy to drop that user's live tunnels (best-effort).
