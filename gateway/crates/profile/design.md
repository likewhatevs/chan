...
yamux registrations on drive-proxy; renames must be capped so the
...
- `drives (id, owner_user_id, drive_name, created_at)` with
  `UNIQUE (owner_user_id, drive_name)`. First-class entity for an
  owner's drive: lets the dashboard list a drive that has no grants
  and no live tunnel yet, and acts as the FK target for grants.
- `drive_grants (id, owner_user_id, drive_name, grantee_email,
  grantee_user_id, role, created_at, accepted_at)` with
  `UNIQUE (owner_user_id, drive_name, lower(grantee_email))` and an
  FK on `(owner_user_id, drive_name)` -> `drives` (cascade delete).
...
profile-service holds a `DriveAdminClient` (from
`gateway_common::drive_admin_client`) when `DRIVE_ADMIN_URL` and
`DRIVE_ADMIN_TOKEN` are set. The block flow fires
`kill_user_tunnels` server-side at the same moment `blocked_at` is
written, so live drive-proxy registrations die without an extra hop
...
### Block fans out to drive-proxy server-side
...
4. Best-effort: if a `DriveAdminClient` is configured, call
   drive-proxy `/admin/v1/users/{username}/tunnels/kill` to evict
...
drive-proxy is the authority on live registrations; profile is the
...
### Drives are first-class

A drive is a row in `drives` keyed on `(owner_user_id, drive_name)`.
The dashboard creates one before either grants land or `chan serve`
registers a live tunnel, so the offline state is always
representable. Live tunnels (held in drive-proxy's in-memory
Registry) reference the same `(owner, drive_name)` pair; the FK
from `drive_grants` -> `drives` makes drive deletion atomic
(cascading every grant on it).

`POST /v1/users/:owner/drives` is idempotent: 201 on insert, 200
when the name already existed. `POST .../grants` upserts the
parent `drives` row in the same transaction (so a caller that
skips the explicit drive-create still produces a valid graph and
the FK never fires).

### Per-drive sharing grants

A user can share one of their drives (the path segment served at
`{owner}.drive.chan.app/{drive}/`) with another user by email.
Grants live in `drive_grants` keyed on
`(owner_user_id, drive_name, lower(grantee_email))`:

- The owner pre-seeds grants from id.chan.app's SPA *before* (or
  alongside) running `chan serve --tunnel-drive=<name>`. The grant
...
- `role` is one of `viewer`, `editor`. Re-adding the same email on
  the same `(owner, drive)` is idempotent: the SQL is
...
`GET /v1/users/:owner/drives/:drive/access?as=<caller_user_id>`
before minting a drive-gate entry JWT. The response is
`{role: "owner"|"editor"|"viewer"}` on access, 404 otherwise. The
404 shape is shared with "unknown drive": neither the access
endpoint nor the share landing page leaks which drives an owner is
sharing.

Drive-name normalization: handler lowercases + trims and rejects
anything outside `[a-z0-9._-]{1,64}` so the stored value is always
the canonical path segment drive-proxy serves. Email uniqueness is
...
`(drive_name, grant_count)` per drive the user has configured shares
on; `GET /v1/users/:id/grants/incoming` returns drives shared *with*
...
- Block always: revokes every active PAT, fires the drive-proxy
  eviction (if configured), appends one `auth_audit` row.
- Bearer comparisons run at constant time.
- `drive_grants.role` is one of `viewer`, `editor` (CHECK
...
- `gateway_common::drive_admin_client::DriveAdminClient` (best-effort
  drive-proxy eviction on admin block)
