...
`<ident>` resolves in this order: a uuid (exact); an email
substring (must match exactly one row); an exact username.

`--json` is available on every command; the default is a
`comfy_table` ASCII table sized for an 80-column terminal.
