#!/usr/bin/env bash
# Generate /etc/chan-gateway/{domain,profile,identity,workspace-proxy,
...
# that must match: profile's PROFILE_ADMIN_TOKEN, workspace-proxy's
...
# exactly two services (identity <-> workspace-proxy).
...
# Generated by configure.sh on $(date -Iseconds).
# workspace-proxy holds no database; hostnames come from domain.env.
