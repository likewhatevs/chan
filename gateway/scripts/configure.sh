#!/usr/bin/env bash
# Generate /etc/chan-gateway/{profile,identity,drive-proxy}.env on a
...
write_env /etc/chan-gateway/drive-proxy.env "$(cat <<EOF
# Generated by configure.sh on $(date -Iseconds).
...
echo "    systemctl enable --now chan-gateway-drive-proxy"
echo
