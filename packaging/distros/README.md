...
- `copr` curls the custom webhook in independent `chan` and `chan-desktop` matrix cells, with fail-fast disabled so both trigger outcomes are reported. Verify `chan` across Fedora plus all four CentOS chroots, and `chan-desktop` across Fedora plus Stream 10. The console denylist should prevent EL9 desktop jobs, but it lapsed for v0.72.0; if one is scheduled, the spec now refuses EL9 immediately and names the unavailable dependencies. Check the job count on every release because the denylist remains console-only state.
