...
`sdme new` deletes the container when its guest command exits non-zero, so a failed target's results directory, not its container, is the surviving diagnostic surface. The guest hands that directory back to the host user on every path, which is what lets the next run clear it; if an older run left it owned by a container uid, the driver says so and names the `sudo rm -rf` that clears it.

`test-build-with-sdme.sh` runs the driver against a stub sdme and a stub guest in seconds: per-target status capture, the re-run after a failed target, interrupt handling, and knob and preflight validation. Run it after editing the driver, since the real matrix costs hours of offline RPM rebuilds.
