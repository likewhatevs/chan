...
#
#   docker/build.sh                 # build all four images, tag :dev
#   docker/build.sh -t v0.49.0-rc1  # custom tag
#   docker/build.sh --save          # also export OCI archives for `sdme fs import`
#   docker/build.sh --model         # build the chan image with the embedded model
#   docker/build.sh -v              # verbose (set -x)
#
# Output OCI archives (with --save) land in docker/_out/<image>.oci.tar, ready
# for: sudo sdme fs import <name> docker/_out/<image>.oci.tar --oci-mode app \
#        --base-fs <base>
set -euo pipefail
