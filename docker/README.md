...
docker build -f docker/gateway.Dockerfile --target identity        -t chan-gateway-identity:dev .
docker build -f docker/gateway.Dockerfile --target profile         -t chan-gateway-profile:dev .
docker build -f docker/gateway.Dockerfile --target devserver-proxy -t chan-gateway-devserver-proxy:dev .
...
docker build -f docker/chan.Dockerfile --build-arg EMBED_MODEL=1 -t chan:model .   # embedded
```
