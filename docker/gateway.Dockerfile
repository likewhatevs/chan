...
#
#   docker build -f docker/gateway.Dockerfile --target identity \
#       -t chan-gateway-identity:dev .
#   docker build -f docker/gateway.Dockerfile --target profile \
#       -t chan-gateway-profile:dev .
#   docker build -f docker/gateway.Dockerfile --target devserver-proxy \
...
# gateway/crates/*/packaging/*.env for the full variable contract and kube/ for
