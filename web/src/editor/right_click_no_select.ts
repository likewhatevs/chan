...
/// Suppress CodeMirror's default mousedown handling for right-
/// clicks (button === 2). Without this, a right-click inside the
/// editor surface triggers a selection gesture (whole line or
/// word, depending on prior state) BEFORE the outer
/// `oncontextmenu` handler in `FileEditorTab.svelte` runs, so
/// the user sees an unintended selection alongside the menu
/// opening.
///
