...
// draft-file-relative (`./image.png`) so the editor preview resolves it. The
// receiving agent reads from the workspace root, so delivery rewrites the ref to
// the workspace-rooted path and strips render-only fragments.
describe("rewriteImagePathsForDelivery", () => {
  ...
  test("no-ops when there is no draft path", () => {
