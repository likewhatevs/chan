...
- That chord must name the TARGET's agent, not your own. `--submit` is resolved in the sender's process and nothing validates it against the target session, so a wrong value fails silently: the text parks in the target's compose box and successive pokes concatenate into it with no separator. Codex and OpenCode share a byte-identical template while Claude's differs, so guessing your own name breaks exactly claude<->codex and claude<->opencode while codex<->opencode appears to work. `CHAN_AGENT` is target-side identity and does not change what a peer sends; `CHAN_MODE` is not read by anything. (v0.73.0)
- **Neither `offline` nor a deep queue means an agent is stuck.** `cs terminal list` reported every tab `offline` while all three agents were alive and working, and separately a lane looked unresponsive to seven messages when its queue depth showed it had drained none of them. Judge a lane by the artifacts it produces (commits, journal writes, worktree changes), confirm liveness with `pgrep` on the agent process, and read the queue depth before concluding anything. Restarting a tab on either signal destroys the agent's context and its uncommitted work. Do not re-send: the queue reconciles the backlog into one turn, so duplicates only make that turn noisier. (v0.73.0)
...
- The same SIGPIPE bites a TAG push, and `rc=0` is not proof. A v0.73.0 tag push reported success, ran all ten gate steps green, and exited 141 with the tag never reaching the remote. Only `git ls-remote --tags` settles it. (v0.73.0)
- **Stage explicit pathspecs even when you are alone in the tree.** Closing v0.73.0 with no peer writing, a `git add <directory>` swept in an editing artifact and a 119-line proposal another agent had left in that directory, and both were committed and pushed without being read. Read `git diff --cached --name-only` BEFORE committing, not after. (v0.73.0)
- When snapshotting a worktree defensively, `git diff` is not enough: it silently omits untracked files, which is exactly where brand-new work lives. Capture `git ls-files --others --exclude-standard` separately, and check `git diff --cached` for staged-but-uncommitted. (v0.73.0)
- If a lane rewrites history after you have merged and pushed it, do NOT merge the rewritten branch: that replays already-merged content as duplicate commits. Verify main's CONTENT is still correct by reading the files, use `git log --cherry-pick --right-only main...<branch>` to find what is genuinely new, cherry-pick only that, and have the lane reset to `origin/main`. (v0.73.0)

## The gate and the quality bar

- Run a scoped own-gate before any "done" report. Rust: `cargo fmt --check` + `cargo clippy --all-targets -D warnings` + `cargo test` (scoped `-p <crate>`; re-check `--no-default-features` if you touched feature gates). Frontend: `make web-check` (vitest, catches stale `?raw` source-pins) + svelte-check + `npm run build`. Desktop: `cd desktop && make build`. Shell or workflow (`packaging/`, `scripts/`, `install.sh`, a git hook, `.github/workflows`): `make shell-check` + `make workflow-check`, which no other scoped gate implies.
...
- Do not suppress stderr on a destructive command either. An `rm -rf target 2>/dev/null` hid whatever stopped it finishing, and the residue was only noticed later by accident. (v0.73.0)
- **Every new check must be provably able to go red, AND must not go red on success.** The first half is the standing rule: break it on purpose once, capture the red, then fix it. The second half is its mirror and is easy to miss. v0.73.0 published both AUR packages successfully and still reported failure, because the post-push verification allowed about fifty seconds for an index that had not caught up. A check that fires on a run that shipped trains the operator to discard it, so the next genuine failure arrives with the same job name and the same error and is waved through. A red that means nothing is worse than no check. (v0.73.0)
- A `publish=false` dry run does not exercise anything gated on publish. `publish-release` and `pages-artifact` are both so gated, which means the release asset verifier and the `/dl` metadata generators are never executed by a rehearsal; their first real run on any change is the GA itself. When changing the published asset set, verify by downloading the dry run's artifacts and diffing the real filenames against the verifier's required list. (v0.73.0)
...
- When re-walking a previously-failed test, `pkill chan open` + rebuild + verify binary provenance + restart. Stale-binary false positives caused multi-round wild-goose chases. In multi-agent runs a broad `pkill` kills every lane's server; the orchestrator serves from a renamed binary copy and lanes scope their pkills to their own path. (phase 15)
...
- Quality bar: keep the full gate green and ship no KNOWN bug. Alex accepts a few small bugs as the solo user but NOT a lower gate. Give a final validation pass and a local DMG before any upstream tag. (phase 16)
