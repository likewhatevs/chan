...
2. **Build + launch**: `cargo build -p chan` rebuilds the binary with the current `web/dist/` bundle, then `./target/debug/chan open <path>` in the background. The URL with the per-launch bearer token lands on stderr.
