...
4. **Dry-run build.** Dispatch `release.yml` on the RC branch with `publish=false`. This is the only way to exercise the macOS sign and notarize path and the Tauri updater signing off a workstation, so it is mandatory before GA. Download the run artifacts and validate them (`cs download` them locally when the build host is remote). Artifact testing can still reject work already merged into the RC.
