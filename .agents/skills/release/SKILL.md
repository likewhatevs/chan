...
8. **Distro packages.** When the Release run completes, the `distros-publish` workflow fires on its own (workflow_run; it cannot block or fail the release): it triggers the COPR builds, uploads the signed PPA sources, and builds then publishes the two AUR recipes on x86_64. Every job in a GA run is expected green. There is no aarch64 cell: the unverified Arch Linux ARM leg (`aur-validate-arm`) is opt-in on manual dispatch, so a red job in a GA run is a real failure. Verify all three went green: COPR `fiorix/chan` (`chan` on every enabled Fedora chroot plus all four CentOS chroots; `chan-desktop` on Fedora plus Stream 10, with no EL9 desktop job), `ppa:fiorix/chan` (noble + resolute), and AUR `chan` + `chan-desktop` at `X.Y.Z-1`. The COPR chroot list, the per-chroot EPEL repository, and the `chan-desktop` EL9 chroot denylist are console state that nothing in the repo asserts and no API call confirms; the release path builds the SCM packages through the webhook, so read them in the COPR web UI whenever a CentOS job is missing, an EL9 job resolves the wrong EPEL, or an EL9 `chan-desktop` job appears at all. A red `aur-validate` cell (build, package-scoped tests, a namcap error-class finding, or the packaged-`chan upgrade` smoke) blocks the AUR push for both packages and nothing else; COPR, the PPA, and the GitHub release are unaffected. It needs `COPR_WEBHOOK`, `LAUNCHPAD_GPG_PRIVATE_KEY`, `LAUNCHPAD_GPG_PASSPHRASE`, and `AUR_SSH_PRIVATE_KEY`. Retry via `workflow_dispatch` with the tag, `publish=true` (dispatch defaults to a dry run), and `targets` narrowed to the distro that failed. Dispatching with `publish=false` before the tag is the way to prove the credentials, including the AUR SSH key, without publishing. See `packaging/distros/README.md`.
