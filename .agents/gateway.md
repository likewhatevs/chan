...
Contribution guidelines for agents and contributors working on the
`gateway/` workspace. Source files live under `gateway/`; this file
documents them from the shared `.agents/` home.

## What this workspace is

The `gateway/` Cargo workspace runs the account, sign-in and
reverse-proxy surface for chan.app. Five crates:
...
- **Crate design references** (canonical; `README.md` next to
  ...
  - [`gateway/crates/profile/design.md`](../gateway/crates/profile/design.md):
    schema, two-tier auth, atomic upsert, block fan-out.
  - [`gateway/crates/identity/design.md`](../gateway/crates/identity/design.md):
    OAuth providers, PAT lifecycle, session contract,
    workspace-gate mint, dashboard.
  - [`gateway/crates/workspace-proxy/design.md`](../gateway/crates/workspace-proxy/design.md):
    apex / wildcard split, workspace-gate verify, registry model,
    reverse-proxy hygiene.
  - [`gateway/crates/admin/design.md`](../gateway/crates/admin/design.md):
    command surface, output contract, exit codes.
  - [`gateway/crates/gateway-common/design.md`](../gateway/crates/gateway-common/design.md):
