...
- The Tauri window watcher now retargets already-open devserver webviews in place when the authoritative launch fields change, so a stable `{library_id}::{window_id}` label keeps the same native window while picking up a rotated per-window tenant token.
