...
- Known limitation on chan-desktop (WKWebView): a `mermaid-to-excalidraw` flowchart with a `subgraph` does not render there, while it renders in the browser. On-device (WKWebView Web Inspector) the error is `Error processing Mermaid diagram: SubGraph element not found`, thrown by `@excalidraw/mermaid-to-excalidraw` while converting the subgraph. It is NOT a CSP or font/WASM issue (the desktop SPA is served over http by chan-server with no CSP header, and there is no font/WASM error). In Blink the same diagram embeds the subgraph as an image and works; WKWebView cannot find the subgraph cluster elements in the offscreen mermaid render, most likely a container-not-attached-or-laid-out difference. A fix is tracked for 0.59.1 (`dev/v0.59.1/excalidraw-chan-desktop.md`); the plain `mermaid` renderer is unaffected.
