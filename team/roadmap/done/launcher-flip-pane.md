...
**Named gap: no live check in a rebuilt server.** The integrator owns this one and did not run it. Reaching it from the browser-smoke harness needs a hybrid pane, and no command creates a B side: `app.pane.splitRight` and `app.pane.splitDown` make new panes, not a hybrid, so the `.side-toggle` glyph the assertion would read never appears without a specific UI interaction. Building that setup is disproportionate to a two-line fix already covered by four tests captured red first, including the case proving the guard still bites with Search open. The residual risk is narrow: the unit tests run against source, so they would not catch a stale `rust-embed` bundle. That is a build-pipeline failure mode rather than a logic one, and any hand-smoke of the launcher's "Flip pane" row on a hybrid pane closes it in seconds.
