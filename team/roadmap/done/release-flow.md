...
1. **Source inventory:** compare the skill's branch, tag, version-pin, report, signing, artifact, and package claims against `.github/workflows/release.yml`, `.github/workflows/publish-downstream.yml`, the Cargo and npm workspaces, `desktop/src-tauri/tauri.conf.json`, `Makefile`, `packaging/`, and the current lockfiles.
