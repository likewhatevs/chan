...
- Give the workflow the release.yml dry-run shape: `publish` defaults to false on manual dispatch, and `targets` scopes a retry to one distro so a transient Launchpad ftp 550 does not also rebuild the Arch packages.
