...
A full `chan` package build ran on a local x86_64 sdme Arch container on 2026-07-19, from a separate probe worktree at a revision that predates this item's namcap-gate fixes, so namcap was advisory in that run rather than enforcing. The package built in 23m47s, passed its release-mode test suite inside the container, and packaged cleanly. namcap reported four warnings and no error-class line: an unused `/usr/lib64/ld-linux-x86-64.so.2` (the dynamic loader), `libgcc` detected and implicitly satisfied, and `gcc-libs` and `systemd` flagged as possibly unneeded. All four are dependency-declaration observations from namcap's soname-only analysis and none is acted on: `gcc-libs` is the package that provides the `libgcc_s.so.1` the same output says the binary needs and there is no `libgcc` package to declare, and `systemd` is a runtime dependency for the packaged user unit and `chan devserver --service=systemd`, which no ELF header can show.

That probe ran in the local sdme Arch rootfs, not in the `docker.io/archlinux/archlinux:base-devel` image `aur-validate` uses, so the gate's own rootfs is unmeasured. The four findings still carry: namcap derives them from the recipe's `depends` and the built binary's sonames, and `chan`'s `depends` has not changed since the probe. So the error-class gate is expected to pass for `chan` with an empty waiver list on measured output rather than on expectation, with the image difference as the one residual. `chan-desktop` namcap output has not been measured anywhere; if it reports an error class the recipe cannot fix, the waiver is one line in `build-in-container.sh`.

The remaining acceptance evidence is deliberately release-bound: full `chan-desktop` Arch builds in the post-release matrix, the CachyOS desktop hand-smoke, and AUR RPC confirmation of both `0.72.0-1` repositories.
