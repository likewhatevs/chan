...
- Extend `distros-publish.yml` with native x86_64 validation for both packages followed by AUR publication. The aarch64 job stays outside `aur-publish`'s `needs` so an unproven ARM cell cannot block shipping the validated x86_64 recipes, and its failure is a visible red job rather than a silent skip. Publication remains post-release and non-gating for the GitHub release.
...
- The v0.72.0 post-release workflow reports the two x86_64 package builds green, the AUR credential probe green, and both AUR repositories at `0.72.0-1`. The two aarch64 cells are reported, not required.
...
The aarch64 leg is the one item that no gate can close before it runs somewhere: this host is x86_64, no runner has executed the ALARM rootfs import or the native ARM Tauri build, and the release path is now shaped so that its absence costs signal rather than blocking publication. Prove it with a `distros-publish` dispatch carrying `targets=aur` and `aur_validate_arm=true`, then fold the result back into this item.
