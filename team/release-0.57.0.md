...
- `chan-systemd` is the Linux-only systemd boundary for notify, fdstore add/remove, inherited named FDs, and fdstore cleanup. Non-Linux builds keep a stable prepare route that returns an explicit unsupported response.
...
- The release workflow is run with `publish=false` before tag push so macOS and Windows build coverage can catch cross-platform breakage from the Linux fdstore changes.
