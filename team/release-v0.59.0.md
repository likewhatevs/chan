...
- Known limitation on chan-desktop (WKWebView): the excalidraw renderer does not render there, while it renders in the browser. It is NOT a CSP problem: on desktop the SPA is served over http by the embedded chan-server (`serve.rs`), which sends no CSP header, and the `tauri.conf.json` CSP does not apply to http-served content, so nothing blocks excalidraw's WASM or workers. Because it works in Blink but not WKWebView against the same server and assets, the cause is a WKWebView-specific failure of excalidraw's harfbuzz-WASM / Web Worker font subsetter (the "falling back to the main thread" path above), which needs on-device Web Inspector debugging to pin down. A fix is tracked for 0.59.1 (brief to land in `dev/v0.59.0/` once the on-device console error is captured); the plain `mermaid` renderer is unaffected.
