...
- **`/api/library/workspaces`** (list + add/on/off/rm) over the `WorkspaceHost` pub API (`open_registered_workspace`, `close_workspace`, …). Mutation is loopback-only (the desktop, single-user, token-gated); read-only over the tunnel (the proxy can't forward a verifiable role -- grantee mutation awaits a signed role header, deferred).
