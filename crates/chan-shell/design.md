...
The server is authoritative over the chord on the write path. `cs terminal write` carries logical text plus the sender's request over the control wire (whether to submit, and the agent NAME the sender believed the target runs; the wire still decodes the former fully-resolved client shape, using only its agent field). At enqueue, the server derives each MATCHED session's agent from that session's own spawn command and `CHAN_AGENT` and resolves the template in the SERVER's environment (env `CHAN_SUBMIT_<AGENT>` > the server's `<config>/chan/submit.toml` > built-in), so a mixed-agent group write delivers a per-session-correct chord, a shell session never receives one, and a `CHAN_SUBMIT_*` value in the writer's environment has no effect. A sender/target disagreement is corrected and reported in the control reply next to the queue position, and `cs terminal list` exposes each session's derived agent so a sender can look the answer up instead of guessing. Rich Prompt resolves the same metadata server-side and enters the queue as one logical message. The queue holds ENTRIES and bounds them at 100: every entry is exactly one raw write, so a Gemini message occupies two and everything else occupies one. Depth counts logical messages (one per tail entry), and that is the number the SPA badge, `cs terminal list --json`, and a `cs terminal write` queue position all report.

`plan_submitted_input` is the drain-time byte source of truth. Gemini remains the split-write agent: a 2026-07-20 live sweep on Gemini 0.51 found no fixed sub-idle gap safe for the required 64 KiB body. At 400 ms the CR still became Shift+Return; at 700 ms the body submitted but the five-block content oracle was not preserved, leaving only 100 ms below the queue's idle threshold. A Gemini message therefore takes TWO queue entries (body, then bare CR) that the normal idle gate separates. The compatibility helpers `apply_submit_chord` and `submit_writes` remain for direct team-spawn callers, which own their own inter-write gap.

At one safe idle opportunity, the terminal drainer selects the maximal consecutive prefix of submitted `cs terminal write` messages with one identical proven built-in submit spec. Two or more messages become one ASCII-framed chronological prompt capped at 64 KiB; one message keeps its exact singleton bytes. Rich Prompt, raw unsubmitted input, Gemini, runtime overrides, agent/spec changes, and the byte ceiling are FIFO boundaries, and the selector never skips them. Codex and OpenCode accept the framed batch in their normal bracketed-paste-plus-CR write. Claude receives the batch body and submit CSI as two atomic controller parts, separated by a gap measured against Claude Code 2.1.215, so paste handling cannot swallow the submit chord.
