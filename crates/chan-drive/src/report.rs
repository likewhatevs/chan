...
//
// chan-drive owns the persisted JSONL (DrivePaths::report), debounces
...
// `Drive::report()` / `Drive::report_jsonl_path()`.
...
/// Per-drive report state. Owned by `Drive` through a `OnceLock`
/// so we pay the initial scan only when the report is actually
/// used (call `Drive::report()` or `Drive::watch()` to warm).
///
/// Three pieces:
        ...
        let outcome = {
            ...
            match ev.kind {
                ...
                WatchKind::ProviderError => {
                    ...
                    // future explicit Drive::rebuild_report().
                    return;
                }
            }
        };
