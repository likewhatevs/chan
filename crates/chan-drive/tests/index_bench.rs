...
//! (graph rebuild + BM25 build_all) via `Drive::reindex`. Mode 2 (WITH
...
//! `Drive::report()` after `set_reports_enabled(true)`, timed on its own.
//!
//! EMBEDDINGS: this benchmark MUST run with the `embeddings` feature
//! compiled OUT (`--no-default-features`). `Drive::reindex` builds with
//! `BuildOptions::default()` (`include_vectors = true`) and there is no
//! public Drive API to reindex BM25-only, so with the feature ON and a
...
fn index_once(cfg_dir: &Path, drive_root: &Path, with_report: bool) -> IndexResult {
    ...
    lib.register_drive(drive_root).unwrap();
    let drive = lib.open_drive(drive_root).unwrap();
