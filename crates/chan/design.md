...
src/devserver_windows.rs     Windows `--service` backend: detached devserver
                             tracked by a PID/state file (systemd/launchd analog)
```
...
`src/devserver_windows.rs` is the `chan devserver --service` backend on Windows, the analog of the systemd user service on Linux and the launchd LaunchAgent on macOS. With no OS supervisor available, it runs a `DETACHED_PROCESS` child tracked by a `service.json` PID/state file; `--stop` / `--restart` locate it by recorded pid plus process-creation-time (a pid-reuse guard) and `TerminateProcess` it. A hard stop is safe because the devserver drains HTTP per request and the writer lock self-heals, the same as a `kill -9`. The detached child re-runs the console `chan.exe` with `ARGV0=chan` so a multiplexed `chan-desktop.exe` still dispatches to the CLI devserver path rather than launching the GUI. It is per-login: it survives the launching shell but not logout, and it does not auto-restart on crash.
