...
Contribution guidelines for Codex (Codex.ai/code) when
working on `chan`.
...
desktop/                Tauri shell. Cross-platform desktop wrapper
                        ...
                        for normal local workspaces and mounts the
                        editor in a webview window. Remote workspaces
...
(Codex, codex, gemini) connect through the in-process MCP server
