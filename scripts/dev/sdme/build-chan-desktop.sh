...
#
#   CHAN_REPO=/path/to/chan DISTRO=ubuntu scripts/dev/sdme/build-chan-desktop.sh
#
...
SDME_TEMPLATE="${CHAN_REPO}/scripts/dev/sdme/${ROOTFS}.sdme"
...
if [ ! -f "$SDME_TEMPLATE" ]; then
    ...
    echo "       (add one per distro under scripts/dev/sdme/)" >&2
    exit 1
fi
