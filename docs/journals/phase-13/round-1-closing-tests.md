...
@@Alex: despite having multiple mentions across various .md files, I do not see any of them in the graph, at all. Perhaps our graph is only considering contacts the .md files with frontmatter kind: contact, because I also realised the @@{mentions} aren't available for searching when I type `@{name}` in the editor: ![](./image-6.png#w=250)
...
@@Alex: clicked on Markdown, it is only showing 1 directory (out of MANY that we have markdown on), and the language itself has no Inspector: ![](./image-7.png#w=250)


  Dashboard carousel (open the Dashboard via Cmd+I or hamburger menu → Infographics)

@@Alex: missing the "Dashboard" option which should be after Graph here: ![](./image-8.png#w=250)

Also the main button still says Infographics, it should say Dashboard: ![](./image-9.png#w=250)
...
@@Alex: the qr-donate is not showing; we must embed this image in our server: ![](./image-10.png#w=250)
...
2. Where is the Screen Lock configuration now? I explicitly asked this to be configured in the settings of the Hybrid Dashboard, which is currently empty: ![](./image-11.png#w=250) 
...
Cmd+, on empty pane flips the whole pane: ![](./image-12.png#w=250)
