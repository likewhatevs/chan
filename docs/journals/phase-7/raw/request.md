...
* The file browser needs a button that allows it to stick to the left or right hand side of the screen in a vertical pane, top to bottom - it lives *outside* of the main pane where we host editor and terminal tabs
  ...
  * The look-and-feel should be familiar (ours already is), and these side-attached file browsers should be inspired by github's: _[screenshot removed: GitHub-style side-attached file browser, look-and-feel reference]_
  * When there is already something in the Find buffer and we didn't close it, we can't press Cmd+F again - does nothing: _[screenshot removed: Cmd+F does nothing when the Find buffer is already open]_; if I close it with the mouse and press Cmd+F again, it opens..
  * The style toolbar for the file editor and the rich prompt must be the exact same, the one from the file editor; we need to upgrade the icon set - are the correct? i dont think so.. judging by the 'insert image' link.. nono
    ...
    * The split left, right, and settings buttons from the terminal menu should resemble the file menu with the sections and separators, same for sequence of items: _[screenshot removed: terminal menu split and settings buttons, current state]_
 vs _[screenshot removed: file menu sections and separators, the target style to match]_
...
* I keep getting eventual timeouts (failed to write after 10s - also, how come? indexing or something else in the way of writing my tiny .md file??? not acceptable), got while writing this very document: _[screenshot removed: failed-to-write-after-10s timeout while editing this document]_
...
* When we've got no matches we should at least indicate that we tried: searched N documents (or, we're indexing, be patient) and in the case of indexing add a spinner before giving up; it should be above the separator which looks weird when empty results return: _[screenshot removed: empty no-matches search view with a stray separator above it]_
* And iirc we have the same issue in the image search dialog triggerd by `![`  - yes _[screenshot removed: same empty-state issue in the image-search dialog]_
...
* The rich input on terminals is missign the right-click button on the rich input area
  ...
  * The current toolbar that we show in the prompt editor should not include that button to toggle source code, that should be only the menu option, and it should resemble the one from file editor _[screenshot removed: file-editor toolbar shape the rich prompt should resemble]_
...
* Editing tet at the end of the page is proven to be very difficult, the page keeps scrolling as I type: _[screenshot removed: page keeps scrolling while typing at the end of the page]_
* Typing on a list moves the cursor before the marker: _[screenshot removed: typing on a list moves the cursor before the marker (1 of 2)]__[screenshot removed: typing on a list moves the cursor before the marker (2 of 2)]_
* switching tabs between doc and terminal results in the editor tab not rendering until click or cursor move: _[screenshot removed: doc-to-terminal tab switch leaves the editor unrendered until click]_
...
* The tab's [BCAST] -> broadcast icon from the menu! can't click mute maybe because 
  ...
  * _Clarification (2026-05-19 02:00 BST, broadcast as a window-wide group):_ BCAST is a **single group per Hybrid window**, not a per-tab construct. All tabs in the same Hybrid see the same group. Membership rules:
    1. Each tab's own **"Broadcast input on"** button is the canonical add/remove for THAT tab. Click once = join the group, click again = leave. Don't put a "self" tick in the broadcast checklist menu — that's confusing and redundant with the tab's own button.
    2. The membership menu lists the OTHER tabs in the Hybrid, with checkboxes to add/remove each. Self is implicit (driven by the per-tab button).
    3. Live bug: today, removing a tab from the group leaves its own "Broadcast input on/off" button stuck off — you can't click it to re-add, and there's no other affordance to bring it back. The button must always be live for its own tab regardless of current membership.
  * _Simplification (2026-05-19 04:00 BST, drop MUTE entirely):_ killing the MUTE concept. Final spec:
    1. **One BCAST group per Hybrid.** All panes in the same Hybrid window share the same group.
    2. **Each tab is in or out.** Binary membership. A tab can be IN by itself even with no other BCAST tabs — perfectly fine.
    3. **No MUTE state.** Remove the per-tab MUTE concept entirely. No `Cmd+Shift+I` mute toggle. No mute-preserved-across-membership logic. Drop the extra strip / bar that had the mute + off buttons.
    4. **Pink indicator on the tab strip** is the only feedback that a tab is in the group. Nothing else needed.
    5. Simplest test: select all → deselect a few → deselect all → select a few. Pink indicator on the right tabs at each step, no other state to worry about.
...
New bug report: _[screenshot removed: new bug report screenshot, no caption preserved]_


Got this after (I think) I clicked to Copy Path of a directory... I needed to reload the whole screen by usign the pane's left-click button _[screenshot removed: error after Copy Path of a directory, needed a full reload]_
 which I thought we had already swapped with the  pane's hamburger _[screenshot removed: the pane left-click reload button]_
