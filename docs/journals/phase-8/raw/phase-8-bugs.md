...
- The graph shows links to files that it says are not in the repo: _[screenshot removed: graph inspector flags an on-disk node as not in the current file listing]_ 
...
- Clicking 'spawn agent' from rich prompt just turns the screen into an overlay, not showing dialog: _[screenshot removed: clicking spawn agent dims the screen to an overlay instead of showing the dialog]_
...
- terminal line adjustment still buggy:
  - iterm: _[screenshot removed: iTerm renders the Claude Code mascot pixel-art glyphs cleanly]_
  - chan's term:  _[screenshot removed: chan terminal renders the same mascot with broken block-element glyphs]_
  ...
  - dispatched as `fullstack-b-29`; shipped 2026-05-22 (commit `b217540`). Audit revealed `customGlyphs` already defaults to `true` in xterm.js 6.x — the hypothesis-A one-liner was a no-op. Real root cause: chan-desktop runs the DOM renderer (no Canvas/WebGL addon loaded) which uses the system font for ALL characters and IGNORES `customGlyphs` entirely; combined with `-b-2`'s `lineHeight: 1.2`, the per-row span has 0.2 × fontSize of vertical space the font's box-drawing glyphs can't fill. Fix: load `@xterm/addon-webgl` (upstream-recommended; canvasAddon is deprecated). WebGL renderer draws pixel-perfect glyphs into the cell rectangle including line-height padding. try/catch + `onContextLoss` disposal handle the rare WebGL-context-unavailable case (silent DOM fallback)
...
- Terminal box-drawing characters render with broken alignment under Source Code Pro + bundled-font default
  ...
  - **partial coverage 2026-05-22 via `fullstack-b-29`** (commit `b217540`): the *rendering* half is solved by loading `@xterm/addon-webgl` so xterm.js's customGlyphs path draws box-drawing + block-element characters pixel-perfect into the cell rectangle (independent of the underlying font's coverage). This alone closes the TUI-alignment user pain for the default ship. STILL OUT-OF-SCOPE for `-b-29`: the broader spec (Source Code Pro behind a cargo feature flag + Settings dropdown for OS-default vs Source Code Pro + per-OS native-mono default fontFamily + download-on-enable flow). The font *bundle* + the Settings UI surface remain @@FullStackB + @@Systacean territory; they ride later when the lean-default-shipping direction lands
...
- Survey-reply echoes to the terminal as `poke<Enter>`; breaks agents that need `poke<Cmd+Enter>`
  ...
  - shipped as `fullstack-b-13` (2026-05-20). Chord-encoding research (empirical WS-frame probe against a live Claude Code session) pinned `\x1b[27;9;13~` (xterm modifyOtherKeys "Cmd+Enter") as the Agent submit chord; codex v0.130.0 accepts `\r` instead — single-chord ship per @@Alex's "if codex fails it's fine, just want the signal" directive. Server-side commit `e24b931` (`SubmitMode { Shell, Agent }` enum + `Session.agent_mode` AtomicBool + `Registry::set_submit_mode` + new `PUT /api/terminal/:session/submit-mode` route + `dispatch_agent_event` branches on the session's mode). SPA-side commit `dce2373` (rich-prompt toolbar toggle between Send + Collapse + `SerTab.rpsm` short-form + AGENT_SUBMIT_CHORD append on Cmd+Enter in agent mode)
...
- Rich prompt submit-mode doesn't survive page reload (server-side state desync)
  ...
  - shipped as `fullstack-b-18` (2026-05-21, commit `f6d498c`). SPA-only fix: after `richPromptFromSer` resolves during tab restore, if the deserialized tab has `submitMode === "agent"` AND a `terminalSessionId`, fire `api.setTerminalSubmitMode(sessionId, "agent")` as fire-and-forget. Server's `Session.agent_mode` defaults false on every spawn; this re-sync closes the desync the bug entry's reproducer was hitting (toolbar reads Agent, server emits Shell chord). Tooltip copy fix piggy-backed on the same commit. 3 new vitest pins cover the re-sync (agent fires PUT, shell skips PUT, no-session skips PUT)
- chan-desktop missing browser-style zoom (Cmd + / - / 0)
  ...
  - shipped as `fullstack-b-19` (2026-05-21, commit `59f5688`). `WindowConfig.zoom_level: f64` field with `#[serde(default = 1.0)]` for legacy config compat. `AppState.live_window_zooms: Mutex<HashMap<String, f64>>` tracks per-window state. Three IPC handlers (`zoom_in` / `zoom_out` / `zoom_reset`) with 10 % step + `[0.25, 5.0]` clamp + `apply_zoom` shared helper. `KEY_BRIDGE_JS` routes `Cmd+=` / `Cmd+-` / `Cmd+0` (plus Numpad variants + Ctrl on Linux/Windows) to the IPC commands. `build_drive_window` accepts a `zoom_seed` parameter applied via `WebviewWindow::set_zoom` after build. Close handler drains the live zoom into `WindowConfig.zoom_level` so the `-b-1` LRU restore picks the level up on next open. 4 new tests (2 config + 2 serve)
...
- Tab right-click "Reload" + "Open Inspector" entries no-op on chan-desktop (macOS)
  ...
  - shipped initially as `fullstack-b-17` (chan-desktop Tauri IPC `reload_window` + `open_devtools` + `Cmd+R` / `Cmd+Opt+I` accelerators via KEY_BRIDGE_JS) + `fullstack-a-36` (SPA-side dispatch via `web/src/api/desktop.ts` helpers + pane-context menu wiring) — both 2026-05-21 in the v0.11.2 mini-wave. Then `fullstack-b-26` (2026-05-22, commit `77c0129`) extended the per-tab right-click menus on FileEditorTab + TerminalTab to surface the same Reload + Open Inspector entries (the original meta-blocker fix only wired the pane-context menu; per-tab right-click was the gap). GraphPanel + FileBrowserSurface deliberately not extended due to label collision with their existing "Reload" entries (graph reload / tree reload); flagged for follow-up if @@WebtestB surfaces the asymmetry
...
- chan-desktop leaves bundled `chan serve` sidecars orphaned after parent dies; new desktop launches can't bind the same drive
  ...
  - dispatched for Round-2 wave-2 against @@FullStackB
  - shipped as `fullstack-b-22` (2026-05-21, commit `3987e73`) — process-group reap on Unix + Drop handler on AppState (defense-in-depth for panic unwind paths) + drive-lock-takeover dialog via new `reclaim_drive_lock` IPC. Heuristic + dialog UX hardening followed in `fullstack-b-25` (2026-05-21, commit `f29611c`); see "chan-desktop orphan-detection heuristic too loose" entry below
...
- Enrich poke event echo with timestamp + task path + heading anchor
  ...
  - **STRONG OBSERVATIONAL EVIDENCE 2026-05-22** — @@Alex tested informally. All four agents (FullStackA, FullStackB, Systacean, CI) were INSTA-rate-limited on bare `poke`. The same agents, prompted with non-bare alternatives, woke up cleanly:
    ...
    The pattern suggests a cache / dedup pathway on Anthropic's side reacting to identical-input repeats. NOT confirmed — the bare-poke + non-bare attempts ran at slightly different times so time-of-day capacity variance isn't ruled out; only Anthropic could confirm via their telemetry. But the evidence is strong enough to act on. Even if the cache-bust theory is partially wrong, enriching the poke text with timestamp + path + heading is a strict improvement for the agent's context. Chicken-and-egg note: agents can't pick up `-21` itself via bare poke if they're rate-limited; @@Alex bootstraps via non-bare prompts until `-21` ships
...
- chan-reports settings toggle missing from Settings UI (regression)
  ...
  - **partial coverage 2026-05-22**: `fullstack-b-28a` (launcher row expand panel, commit `c5315fd`) + `fullstack-b-28b` slice iii (drive-add pre-flight modal, commit `defbdcc`) ship the reports toggle at two chan-desktop surfaces (per-drive). Sets state via `chan reports enable/disable --path <p>` per slice i (commit `0ce975b`); reads authoritative state via `chan index status --json` per slice ii (commit `efd7688`). What's still missing per this bug body: the SETTINGS UI toggle (Settings panel or Hybrid back-side per @@Alex's open question #2). The launcher + pre-flight surfaces are chan-desktop-only; the Settings UI lives in the SPA + is independently routed to `@@FullStackA` for the graph-overhaul wave
  - NOT YET DISPATCHED (for Settings UI surface) — folds into the graph-overhaul wave
...
- chan-desktop orphan-detection heuristic too loose (false-positive risk in noisy shell environments)
  ...
  - dispatched as `fullstack-b-25`; shipped 2026-05-21 (commit `f29611c`). Tightened heuristic from "three independent substring contains" to a positional argv check (`argv[0]` basename must be exactly `chan`, `argv[1]` must be `serve`, drive key must be a standalone token in `argv[2..]`). Rejects wrapper invocations (`strace chan serve <key>`, `lldb -p N chan serve <key>`, shell wrappers) and path-substring matches. New `OrphanCandidate { pid, command }` struct + new `find_drive_lock_candidates` IPC surface the candidate set to the SPA; reclaim dialog replaced Tauri's opaque `ask()` with a custom modal listing per-row PID + command line so the user sees exactly what would be SIGTERM'd before confirming. 3 new heuristic-rejection unit tests + 2 new IPC structural pins. chan-desktop 43 → 47
